...
void HashTable::SaveToFile(const char* fn) {
	...
	fs.write(reinterpret_cast<char*>(&size_), sizeof(size_));
	for (size_t i = 0; i < size_; i++)
		if (table_[i]) {
			Student* temp = table_[i];
			while (temp) {
				...
				fs.write(temp->GetName().c_str(), len);
				fs.write(temp->GetDate(), 10);
				len = temp->GetGroup().length();
				fs.write(reinterpret_cast<char*>(&len), sizeof(len));
				fs.write(temp->GetGroup().c_str(), len);
				temp = temp->next;
			}
		}
	fs.close();
...
void HashTable::LoadFromFile(const char* fn) {
	...
	Clear();
	delete[] table_;
	fcin.read((char*)&size_, sizeof(size_));
	table_ = new Student * [size_] {0};
	while (true) {
		int len = 0;
		if (!fcin.read((char*)&len, sizeof(len))) break;
		char* name = new char[len + 1]{ 0 };
		fcin.read(name, len);
		char* date = new char[11]{ 0 };
		fcin.read(date, 10);
		fcin.read((char*)&len, sizeof(len));
		char* group = new char[len + 1]{ 0 };
		fcin.read(group, len);
		Push(name, date, group);
		delete[]name; delete[]date; delete[]group;
	}
	fcin.close();
}
